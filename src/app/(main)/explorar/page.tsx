...
export default async function ExplorarPage({
  ...
  return (
    <div className="flex flex-col">
      ...
      {hayFiltros && (
        <section className="flex flex-col gap-4 pb-2 pt-4">
          ...
          <form method="GET" action="/explorar" className="flex flex-col gap-3 sm:flex-row">
            ...
            <select name="carrera_id" defaultValue={params.carrera_id ?? ''} className="h-10 rounded-md border border-linea bg-papel px-3 font-mono text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo">
              <option value="">Todas las carreras</option>
              {carreras?.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
            </select>
            <select name="corte" defaultValue={params.corte ?? ''} className="h-10 rounded-md border border-linea bg-papel px-3 font-mono text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo">
            ...
            <select name="orden" defaultValue={orden} className="h-10 rounded-md border border-linea bg-papel px-3 font-mono text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo">
      ...
      {hayFiltros ? (
        <ResultadosBusqueda params={params} orden={orden} anonId={anonId} loggedIn={!!user} userId={user?.id ?? null} />
      ) : (
        <PaginaInicio carreras={carreras ?? []} anonId={anonId} loggedIn={!!user} userId={user?.id ?? null} />
      )}
    </div>
  )
}

async function ResultadosBusqueda({ params, orden, anonId, loggedIn, userId }: { params: SearchParams; orden: OrdenDocumentos; anonId: string | null; loggedIn: boolean; userId: string | null }) {
  const supabase = await createClient()
  const { data: documentos } = await supabase.rpc('buscar_documentos', {
    p_query: params.q || null, p_carrera_id: params.carrera_id || null, p_materia_id: params.materia_id || null,
    p_semestre: params.semestre || null, p_corte: params.corte || null, p_orden: orden, p_anon_id: anonId, p_limit: 24, p_offset: 0,
  })

  if (!documentos || documentos.length === 0) {
    return <div className="flex flex-col items-center gap-3 py-16 text-center"><span className="font-mono text-4xl text-linea">?</span><p className="text-tinta-suave">{params.q ? `No encontramos parciales para "${params.q}"` : 'No encontramos parciales con esos filtros.'}</p><Link href="/subir" className="mt-2"><Button variant="accent" size="sm">Subir un parcial</Button></Link></div>
  }
...
async function PaginaInicio({ carreras, anonId, loggedIn, userId }: { carreras: { id: string; nombre: string; color: string }[]; anonId: string | null; loggedIn: boolean; userId: string | null }) {
  ...
  const porCarrera = await Promise.all(carreras.map(async (carrera) => {
    const [{ data: documentos }, { data: total }] = await Promise.all([
      supabase.rpc('buscar_documentos', { p_carrera_id: carrera.id, p_orden: 'recientes', p_anon_id: anonId, p_limit: CANTIDAD_POR_CARRERA, p_offset: 0 }),
  ...
  if (destacadosDocs.length === 0 && carrerasConDocumentos.length === 0) {
    return <div className="flex flex-col items-center gap-3 py-16 text-center"><span className="font-mono text-4xl text-linea">?</span><p className="text-tinta-suave">Aún no hay parciales. ¡Sé el primero en subir uno!</p><Link href="/subir" className="mt-2"><Button variant="accent" size="sm">Subir un parcial</Button></Link></div>
  }
  ...
  return (
    <div className="flex flex-col pb-10">
      {destacadosDocs.length > 0 && (
        <section className="pt-8">
          <div className="mb-4 flex items-baseline justify-between">
            <h2 className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-lapiz-rojo">Recién subidos</h2>
            <Link href="/explorar?orden=recientes" className="text-xs text-tinta-suave hover:text-tinta hover:underline">Ver todos →</Link>
          </div>
      ...
      <div className="-mx-4 mt-6 px-4 sm:mt-8">
        <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {Array.from({ length: 5 }).map((_, i) => (
            <span key={i} aria-hidden="true" className="h-9 w-28 shrink-0 rounded-md border border-linea bg-white shadow-paper-sm sm:w-32" />
          ))}
        </div>
      </div>
      ...
      {carrerasConDocumentos.length > 0 && (
        <section className="mt-10">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div className="flex gap-1.5"><span className="h-2 w-2 rounded-full border border-linea" /><span className="h-2 w-2 rounded-full border border-linea" /><span className="h-2 w-2 rounded-full border border-linea" /></div>
          </div>
      ...
      <section className="mt-2 flex flex-col gap-4 rounded-md bg-tinta px-5 py-6 text-papel shadow-paper-sm sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><h2 className="font-mono text-lg font-bold">¿Ya cursaste esa materia?</h2><p className="mt-1 text-xs text-papel/70">Sube el parcial y ayuda a quien viene después.</p></div><Link href="/subir" className="inline-flex h-10 shrink-0 items-center justify-center rounded-md bg-resaltador px-5 text-sm font-semibold text-tinta transition-transform hover:-translate-y-0.5">Subir mi parcial</Link></section>
    </div>
  )
}
