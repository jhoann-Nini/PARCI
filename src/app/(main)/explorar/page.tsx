...
export default async function ExplorarPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
...
async function ResultadosBusqueda({ params, orden, anonId, loggedIn, userId }: { params: SearchParams; orden: OrdenDocumentos; anonId: string | null; loggedIn: boolean; userId: string | null }) {
  const supabase = await createClient()
  const { data: documentos } = await supabase.rpc('buscar_documentos', { p_query: params.q || null, p_carrera_id: params.carrera_id || null, p_materia_id: params.materia_id || null, p_semestre: params.semestre || null, p_corte: params.corte || null, p_orden: orden, p_anon_id: anonId, p_limit: 24, p_offset: 0 })
  if (!documentos || documentos.length === 0) return <div className="flex flex-col items-center gap-3 py-16 text-center"><span className="font-mono text-4xl text-linea">?</span><p className="text-tinta-suave">{params.q ? `No encontramos parciales para "${params.q}"` : 'No encontramos parciales con esos filtros.'}</p><Link href="/subir" className="mt-2"><Button variant="accent" size="sm">Subir un parcial</Button></Link></div>
      ...
      {destacadosDocs.length > 0 && (
        <section>
          <div className="mb-4 flex items-baseline justify-between">
            <div>
              <h2 className="flex items-center gap-2 font-mono text-lg font-bold text-tinta"><span className="inline-block h-1.5 w-1.5 rounded-full bg-lapiz-rojo" />Recién subidos</h2>
              <p className="mt-1 text-xs text-tinta-suave">Lo último que subieron tus compañeros.</p>
            </div>
            <Link href="/explorar?orden=recientes" className="text-xs text-lapiz-rojo hover:underline">Ver todos →</Link>
          </div>
            ...
            {destacadosDocs.map((doc, i) => (
              <RevelarAlEntrar key={doc.id} retrasoMs={i * 80}>
                <CardDestacada fechaSubida={doc.fecha_subida} index={i as 0 | 1 | 2}>
                  <ExamenCard id={doc.id} materia={doc.materia_nombre} carrera={doc.carrera_nombre} carreraColor={doc.carrera_color as ColorCarrera} semestre={doc.semestre} corte={doc.corte} temas={doc.temas} votosCount={doc.votos_count} yaVoto={doc.ya_voto} comentariosCount={doc.comentarios_count} loggedIn={loggedIn} esDueno={!!userId && doc.subido_por === userId} />
                </CardDestacada>
              </RevelarAlEntrar>
            ))}
          </div>
