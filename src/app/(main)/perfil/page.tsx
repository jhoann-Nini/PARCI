...
export default async function PerfilPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const docs = (favoritosRows ?? []).map((row: FavoritoRow) => {
    ...
    return d && o && m
      ? { id: d.id, materia: m.nombre, carrera: c?.nombre ?? 'Carrera', carreraColor: (c?.color ?? 'aula') as ColorCarrera, semestre: o.semestre, corte: d.corte, temas: null, votosCount: 0, yaVoto: false, comentariosCount: 0, loggedIn: true, esDueno: false }
      : null
        ...
        <div className="mx-auto w-full max-w-3xl px-5 pb-0 pt-8 sm:px-8 sm:pt-10">
          <div className="flex items-start gap-4 sm:gap-5">
            <div className="relative shrink-0">
              <div className="flex h-16 w-16 items-center justify-center rounded-md border-2 border-azul-aula bg-azul-aula font-serif text-2xl font-bold text-papel sm:h-20 sm:w-20 sm:text-3xl">
                {iniciales || <UserRound />}
              </div>
              <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 rounded-sm bg-resaltador px-2 py-0.5 font-mono text-[9px] font-bold uppercase text-[#4A3800]">
                {perfil?.rol === 'estudiante' ? 'Estudiante' : perfil?.rol ?? 'Estudiante'}
              </span>
            </div>
        ...
        {pestaña === 'cuenta' ? (
          ...
          <section>
            <div className="mb-5 flex items-center justify-between gap-3">
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-tinta-suave">
                {pestaña === 'guardados' ? 'Parciales guardados' : `${subidos ?? 0} parciales subidos`}
              </p>
...
function ProfileStat({ value, label }: { value: number; label: string }) {
  return (
    <div className="border-r border-white/15 py-4 text-center last:border-r-0">
      <strong className="font-serif text-2xl text-resaltador">{value}</strong>
      <p className="mt-0.5 font-mono text-[8px] uppercase tracking-[0.16em] text-papel/55">{label}</p>
    </div>
  )
}
...
function ProfileTab({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link href={href} className={`border-b-2 px-4 py-3 text-xs font-medium transition-colors sm:px-6 sm:text-sm ${active ? 'border-lapiz-rojo font-semibold text-tinta' : 'border-transparent text-tinta-suave hover:text-tinta'}`}>
      {children}
    </Link>
  )
}
...
function InfoCard({ title, rows }: { title: string; rows: [string, string][] }) {
  return (
    <Card className="overflow-hidden">
      <div className="border-b border-linea px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-tinta-suave">{title}</div>
      {rows.map(([label, value]) => (
        <div key={label} className="flex min-h-11 items-center justify-between gap-4 border-b border-linea px-3 py-2 text-xs last:border-b-0">
          <span className="text-tinta-suave">{label}</span>
          <span className="max-w-[65%] truncate text-right font-medium text-tinta">{value}</span>
        </div>
      ))}
    </Card>
  )
...
function Empty({ text, action }: { text: string; action?: string }) {
  return (
    <Card className="p-8 text-center">
      ...
      {action && <Link href="/subir" className="mt-4 inline-block"><Button variant="accent" size="sm">{action}</Button></Link>}
    </Card>
  )
}
