  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        ...
        <select required value={carreraId} onChange={(e) => handleCarreraChange(e.target.value)} className="h-10 rounded-md border border-linea bg-papel px-3 text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select required value={materiaId} onChange={(e) => setMateriaId(e.target.value)} disabled={!carreraId || materias.length === 0} className="h-10 rounded-md border border-linea bg-papel px-3 text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo disabled:opacity-50">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select required value={semestre} onChange={(e) => setSemestre(e.target.value)} className="h-10 rounded-md border border-linea bg-papel px-3 text-sm text-tinta focus:outline-2 focus:outline-lapiz-rojo">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-4 gap-2">
          {CORTES.map((c) => (
            <button key={c.value} type="button" onClick={() => setCorte(c.value)} className={`h-10 rounded-md border font-mono text-sm font-medium transition-colors ${corte === c.value ? 'border-lapiz-rojo bg-lapiz-rojo text-papel' : 'border-linea bg-papel text-tinta hover:border-tinta-suave'}`}>
              {c.label}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="font-mono text-sm font-medium text-tinta">Archivo</label>
        <label className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed border-linea p-6 transition-colors hover:border-tinta-suave">
          <span className="font-mono text-2xl text-tinta-suave">ARCHIVO</span>
          <span className="text-center text-xs text-tinta-suave">
            {archivo ? archivo.name : `PDF, JPG, PNG, WEBP, DOC, DOCX, XLS, XLSX, PPT o PPTX · máx. ${MAX_ARCHIVO_MB}MB`}
          </span>
          <input
            type="file"
